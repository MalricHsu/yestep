...
const PopularTrails = ({ onUpdateSuccess, hasBorder = false }) => {
  ...
  useEffect(() => {
    const getPopularTrailScenery = async () => {
      try {
        const res = await TrailsApi.get('/trails?_sort=trail_popular&_limit=10&_order=desc');
  ...
  return (
    <section className="popularTrails pb-8 py-sm-16" id="popular-trails">
      <div className="container">
        <div className={`pt-8 ${hasBorder ? 'border-1 border-top border-primary-200' : ''}`}>
          <div className="d-flex justify-content-between align-items-center mb-4 mb-sm-8">
            <h2 className="text-black-900 mb-2 mb-sm-0 fs-5 fs-sm-2">本週熱門步道推薦</h2>
            <div className={`d-none d-sm-flex gap-3 ${isLoading ? 'opacity-0' : 'isLoaded'}`}>
                ...
                return (
                  <div className="swiper-slide" key={trail.id}>
                    <Link
                      ...
                      onClick={() => handleAddPopular(trail.id, trail.trail_popular)}
                    >
                      ...
                      <div className="card-body d-flex flex-column align-items-start">
                        ...
                        <div className="d-flex justify-content-between align-items-end w-100 mt-auto">
                          <div className="d-flex flex-column">
                            ...
                            <p className="text-black-100 fs-9 mb-1">{trail.trail_address}</p>
