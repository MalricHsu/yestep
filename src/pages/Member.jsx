import { useEffect } from 'react';
import Nav from '../components/Nav';

const Member = () => {
    useEffect(() => {
        document.title = '會員中心 | YeStep';
    }, []);
    return (
        <>
            <Nav />
            <section className="">
                <ul
                    class="nav nav-underline opacity-75 d-flex
                "
                    style={{}}
                >
                    <li
                        class="nav-item flex-grow-1 flexHover"
                        style={{ minWidth: 'fit-content', flex: '1 1 auto' }}
                    >
                        <button
                            type="button"
                            class="nav-link body1-medium w-100"
                            aria-current="page"
                        >
                            {/* 會員中心 */}
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                height="24px"
                                viewBox="0 -960 960 960"
                                width="24px"
                                fill="currentColor"
                            >
                                <path d="M234-276q51-39 114-61.5T480-360q69 0 132 22.5T726-276q35-41 54.5-93T800-480q0-133-93.5-226.5T480-800q-133 0-226.5 93.5T160-480q0 59 19.5 111t54.5 93Zm146.5-204.5Q340-521 340-580t40.5-99.5Q421-720 480-720t99.5 40.5Q620-639 620-580t-40.5 99.5Q539-440 480-440t-99.5-40.5ZM480-80q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm100-95.5q47-15.5 86-44.5-39-29-86-44.5T480-280q-53 0-100 15.5T294-220q39 29 86 44.5T480-160q53 0 100-15.5ZM523-537q17-17 17-43t-17-43q-17-17-43-17t-43 17q-17 17-17 43t17 43q17 17 43 17t43-17Zm-43-43Zm0 360Z" />
                            </svg>
                        </button>
                    </li>
                    <li class="nav-item flex-grow-1" style={{ minWidth: 'fit-content' }}>
                        <button
                            type="button"
                            class="nav-link body1-medium w-100"
                            aria-current="page"
                        >
                            {/* 我的收藏 */}
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                height="24px"
                                viewBox="0 -960 960 960"
                                width="24px"
                                fill="currentColor"
                            >
                                <path d="m480-120-58-52q-101-91-167-157T150-447.5Q111-500 95.5-544T80-634q0-94 63-157t157-63q52 0 99 22t81 62q34-40 81-62t99-22q94 0 157 63t63 157q0 46-15.5 90T810-447.5Q771-395 705-329T538-172l-58 52Zm0-108q96-86 158-147.5t98-107q36-45.5 50-81t14-70.5q0-60-40-100t-100-40q-47 0-87 26.5T518-680h-76q-15-41-55-67.5T300-774q-60 0-100 40t-40 100q0 35 14 70.5t50 81q36 45.5 98 107T480-228Zm0-273Z" />
                            </svg>
                        </button>
                    </li>
                    <li class="nav-item flex-grow-1" style={{ minWidth: 'fit-content' }}>
                        <button
                            type="button"
                            class="nav-link body1-medium w-100"
                            aria-current="page"
                        >
                            {/* 統計分析 */}
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                height="24px"
                                viewBox="0 -960 960 960"
                                width="24px"
                                fill="currentColor"
                            >
                                <path d="M160-160v-320h160v320H160Zm240 0v-640h160v640H400Zm240 0v-440h160v440H640Z" />
                            </svg>
                        </button>
                    </li>
                    <li class="nav-item flex-grow-1" style={{ minWidth: 'fit-content' }}>
                        <button
                            type="button"
                            class="nav-link body1-medium w-100"
                            aria-current="page"
                        >
                            {/* 你可能會喜歡... */}
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                height="24px"
                                viewBox="0 -960 960 960"
                                width="24px"
                                fill="currentColor"
                            >
                                <path d="M200-200v-560 179-19 400Zm80-240h221q2-22 10-42t20-38H280v80Zm0 160h157q17-20 39-32.5t46-20.5q-4-6-7-13t-5-14H280v80Zm0-320h400v-80H280v80Zm-80 480q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h560q33 0 56.5 23.5T840-760v258q-14-26-34-46t-46-33v-179H200v560h202q-1 6-1.5 12t-.5 12v56H200Zm409-229q-29-29-29-71t29-71q29-29 71-29t71 29q29 29 29 71t-29 71q-29 29-71 29t-71-29ZM480-120v-56q0-24 12.5-44.5T528-250q36-15 74.5-22.5T680-280q39 0 77.5 7.5T832-250q23 9 35.5 29.5T880-176v56H480Z" />
                            </svg>
                        </button>
                    </li>
                </ul>
            </section>
        </>
    );
};

export default Member;
